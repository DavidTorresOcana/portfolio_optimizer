...
def optimize_portfolio():
    # Get arguments
    parser = argparse.ArgumentParser(description="A program to optimize a portfolio comprised by the provided list of symbols by using their historical data")

    parser.add_argument('input_file', type=str,
                        help='Input .txt file with symbols to use in different lines')
    parser.add_argument('output_path', type=str,
                        help='Path to where to place output CSV files with optimized allocations')
    parser.add_argument("-D", "--data_source",
                      help="Data source. Any of 'yahoo' or 'quandl'", default="yahoo")
    parser.add_argument("-s", "--start_date",
                      help=f"Start date: Begining of data to use for optimization with format 'YYYY/MM/DD'", default="2020/01/01")
    parser.add_argument("-e", "--end_date",
                      help=f"End date: End date of data to use for optimization with format 'YYYY/MM/DD'", default="2021/06/30")
    parser.add_argument("--allow_sorting", help=f"Whether to allow sorting or not",
                      action="store_true")
    parser.add_argument("-q", "--quiet",
                      action="store_false", dest="verbose", default=True,
                      help="Do not show graphics, only outputs")

    args = parser.parse_args()
    if not os.path.isfile(args.input_file) or not os.path.exists(args.input_file):
        raise Exception(f"Path to input_file '{args.input_file}' does not exists")
    INPUT_SYMBOLS_FILEPATH = args.input_file
    DATA_SOURCE = args.data_source
    try:
        datetime.datetime.strptime(args.start_date, DATE_FORMAT)
    except Exception as e:
        raise Exception(f"{args.start_date} must be of format {DATE_FORMAT}" + e)
    START_DATE = args.start_date
    SHORTING_ALLOWED = args.allow_sorting
